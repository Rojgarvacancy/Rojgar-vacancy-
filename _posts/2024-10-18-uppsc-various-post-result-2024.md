---
layout: default
title: "UPPSC Various Post Recruitment Result 2024 Declared 2026,PDF"
date: 2026-09-21 11:00:00 +0530
categories: ["Result"]
---

<div style="color: #222222; font-family: Arial, Helvetica, sans-serif; line-height: 1.6; margin: 0px auto; max-width: 900px;">

  <!--Top Red Header-->
  <div style="background: rgb(198, 40, 40); border-radius: 4px 4px 0px 0px; color: white; font-size: 20px; font-weight: bold; padding: 12px 15px; text-align: center;">
    Uttar Pradesh Public Service Commission (UPPSC)
  </div>

  <!--Short Info-->
  <div style="background: rgb(255, 248, 225); border: 1px solid rgb(255, 204, 128); margin-bottom: 15px; padding: 12px 15px;">
    <strong>Short Information :</strong> Uttar Pradesh Public Service Commission (UPPSC, Prayagraj) has officially declared the <strong>Selection Result / Merit List PDF</strong> for Various Post Direct Recruitment 2024 (Advt No. A-4/E-1/2024). Candidates who appeared in the examination/interview can now download their result and check their roll numbers using the direct link provided below.
  </div>

  <!--Important Dates & Application Fee Table-->
  <table style="border-collapse: collapse; font-size: 14px; margin-bottom: 15px; width: 100%;">
    <thead>
      <tr style="background: rgb(21, 101, 192); color: white;">
        <th style="border: 1px solid rgb(144, 202, 249); padding: 8px; width: 50%; text-align: left;">Important Dates</th>
        <th style="border: 1px solid rgb(144, 202, 249); padding: 8px; width: 50%; text-align: left;">Application Fee</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px; vertical-align: top;">
          • Application Begin : <strong>18/10/2024</strong><br>
          • Last Date for Apply Online : <strong>18/11/2024</strong><br>
          • Last Date Pay Exam Fee : <strong>18/11/2024</strong><br>
          • Last Date Correction : <strong>25/11/2024</strong><br>
          • Result Declared : <span style="color: red; font-weight: bold;">Available Now</span>
        </td>
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px; vertical-align: top;">
          • General / OBC / EWS : <strong>₹105/-</strong><br>
          • SC / ST : <strong>₹65/-</strong><br>
          • PH (Divyang) : <strong>₹25/-</strong><br><br>
          <em>Pay the Examination Fee Through Debit Card, Credit Card, Net Banking or UPI Mode.</em>
        </td>
      </tr>
    </tbody>
  </table>

  <!--Age Limit-->
  <h3 style="background: rgb(21, 101, 192); color: white; font-size: 16px; margin: 0px; padding: 8px 12px;">UPPSC Various Post : Age Limit Details (as on 01/07/2024)</h3>
  <div style="background: #ffffff; border: 1px solid rgb(144, 202, 249); margin-bottom: 15px; padding: 10px 15px; font-size: 14px;">
    • Minimum Age : <strong>21 Years</strong> (Post Wise)<br>
    • Maximum Age : <strong>40 Years</strong> (Post Wise)<br>
    • <em>Age Relaxation Extra as per UPPSC Direct Recruitment Rules 2024.</em>
  </div>

  <!--Exam & Qualification Details-->
  <h3 style="background: rgb(21, 101, 192); color: white; font-size: 16px; margin: 0px; padding: 8px 12px;">Vacancy Details & Post-Wise Eligibility (Total : 109 Posts)</h3>
  <table style="border-collapse: collapse; font-size: 14px; margin-bottom: 15px; width: 100%;">
    <thead>
      <tr style="background: rgb(227, 242, 253);">
        <th style="border: 1px solid rgb(144, 202, 249); padding: 8px; text-align: left; width: 35%;">Post Name</th>
        <th style="border: 1px solid rgb(144, 202, 249); padding: 8px; text-align: center; width: 15%;">Total Posts</th>
        <th style="border: 1px solid rgb(144, 202, 249); padding: 8px; text-align: left; width: 50%;">Eligibility Criteria</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px;"><strong>Assistant Chemist</strong></td>
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px; text-align: center; font-weight: bold; color: green;">02</td>
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px;">Master Degree in Chemistry (M.Sc) with 2 Years Relevant Experience.</td>
      </tr>
      <tr style="background: rgb(249, 249, 249);">
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px;"><strong>Lecturer (Various Subjects)</strong></td>
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px; text-align: center; font-weight: bold; color: green;">08</td>
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px;">Five Years Degree / Master Degree in Ayurveda / Unani or Relevant Subject.</td>
      </tr>
      <tr>
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px;"><strong>Reader (Various Subjects)</strong></td>
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px; text-align: center; font-weight: bold; color: green;">04</td>
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px;">Degree in Ayurveda / Unani with Minimum 7 Years Teaching Experience.</td>
      </tr>
      <tr style="background: rgb(249, 249, 249);">
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px;"><strong>Medical Officer (Allopath)</strong></td>
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px; text-align: center; font-weight: bold; color: green;">95</td>
        <td style="border: 1px solid rgb(144, 202, 249); padding: 8px;">MBBS Degree from Recognized University & Registered with Medical Council.</td>
      </tr>
    </tbody>
  </table>

  <!--Important Links-->
  <h3 style="background: rgb(198, 40, 40); color: white; font-size: 16px; margin: 0px; padding: 8px 12px;">Some Useful Important Links</h3>
  <table style="border-collapse: collapse; font-size: 14px; margin-bottom: 20px; width: 100%;">
    <tbody>
      <tr style="background: rgb(255, 235, 238);">
        <td style="border: 1px solid rgb(239, 154, 154); padding: 10px; width: 50%;"><strong>Download Result / Merit List PDF</strong></td>
        <td style="border: 1px solid rgb(239, 154, 154); padding: 10px; text-align: center;">
          <a href="https://uppsc.up.nic.in/Open_PDF.aspx?I4PnQ0tBagmNKfqXmoSUnj3RSz4YcAsN" target="_blank" rel="noopener noreferrer" style="background: rgb(46, 125, 50); border-radius: 4px; color: white; font-weight: bold; padding: 6px 14px; text-decoration: none; display: inline-block;">Click Here</a>
        </td>
      </tr>
      <tr>
        <td style="border: 1px solid rgb(239, 154, 154); padding: 10px;"><strong>UPPSC Official Website</strong></td>
        <td style="border: 1px solid rgb(239, 154, 154); padding: 10px; text-align: center;">
          <a href="https://uppsc.up.nic.in/" target="_blank" rel="noopener noreferrer" style="background: rgb(239, 108, 0); border-radius: 4px; color: white; font-weight: bold; padding: 6px 14px; text-decoration: none; display: inline-block;">Click Here</a>
        </td>
      </tr>
      <tr style="background: rgb(255, 235, 238);">
        <td style="border: 1px solid rgb(239, 154, 154); padding: 10px;"><strong>Photo & Signature Resizer Tool</strong></td>
        <td style="border: 1px solid rgb(239, 154, 154); padding: 10px; text-align: center;">
          <a href="https://photoresizer.store" target="_blank" style="background: rgb(106, 27, 154); border-radius: 4px; color: white; font-weight: bold; padding: 6px 14px; text-decoration: none; display: inline-block;">Click Here</a>
        </td>
      </tr>
      <tr>
        <td style="border: 1px solid rgb(239, 154, 154); padding: 10px;"><strong>Support Desk (Email)</strong></td>
        <td style="border: 1px solid rgb(239, 154, 154); padding: 10px; text-align: center;">
          <a href="mailto:rojgarvecancy.in@gmail.com" style="background: rgb(46, 125, 50); border-radius: 4px; color: white; font-weight: bold; padding: 6px 14px; text-decoration: none; display: inline-block;">rojgarvecancy.in@gmail.com</a>
        </td>
      </tr>
    </tbody>
  </table>

  <!--Clean Disclaimer Note-->
  <div style="background: rgb(245, 245, 245); border-left: 4px solid rgb(198, 40, 40); color: #555555; font-size: 13px; padding: 10px 15px;">
    <strong>Note:</strong> Selected candidates are advised to verify their details with the official UPPSC notification and merit list PDF. Keep checking the official portal for further joining and document verification instructions.
  </div>

</div>
